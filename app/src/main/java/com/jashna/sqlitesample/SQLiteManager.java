...
//        }
    ...
    public void addNoteToDatabase(NotePojo note)
    {
    ...
    public void updateNoteInDB(NotePojo note){
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        ...
        try (Cursor result = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLE_NAME, null))
        {
            if (result.getCount()!= 0){
                while (result.moveToNext()){
                    ...
                    String strdeleted = result.getString(4);
                    Date deleted = getDateFromString(strdeleted);
                    NotePojo notePojo = new NotePojo(id, title, desc, deleted);
                    NotePojo.noteArrayList.add(notePojo);
                }
            }
        }
